...
export class FormControl<T = any> extends AbstractControl<T|null> {
  ...
  */
  ...
  reset(formState: T|{
    value?: T|null;
    disabled?: boolean
  }|null = null, options: {onlySelf?: boolean, emitEvent?: boolean} = {}): void {
    this._applyFormState(formState);
...
/**
 * Given a control "TControl", returns the type of TControl.value
 */
type TypeFromControl<TControl> = TControl extends FormGroup<infer U>? {
  [K in keyof U]: TypeFromControl<U[K]>;
}
  : TControl extends AbstractControl<infer W>
  ? W
  : any;
   ...
   * @publicApi
   */
  export class FormGroup<TGroup extends{[key: string]: AbstractControl} = {
    [key: string]: AbstractControl
  }> extends AbstractControl < {
    [K in keyof TGroup]: TypeFromControl<TGroup[K]>;
  }
  > {
    /**
    ...
    */
     ...
     */
    registerControl<K extends keyof TGroup>(name: K, control: TGroup[K]): TGroup[K] {
     ...
     * @param control Provides the control for the given name
     */
    addControl<K extends keyof TGroup>(name: K, control: TGroup[K]): void {
     ...
     */
    removeControl<K extends keyof TGroup>(name: K): void {
     ...
     * @param control Provides the control for the given name
     */
    setControl<K extends keyof TGroup>(name: K, control: TGroup[K]): void {
      if (this.controls[name]) this.controls[name]._registerOnCollectionChange(() => {});
      delete (this.controls[name]);
     ...
     */
    contains<K extends keyof TGroup>(controlName: K): boolean {
     ...
     */
    setValue(
        value: {[K in keyof TGroup]: TypeFromControl<TGroup[K]>},
     ...
     */
    patchValue(
        value: Partial<{[K in keyof TGroup]: TypeFromControl<TGroup[K]>}>,
        options: {onlySelf?: boolean, emitEvent?: boolean} = {}): void {
      Object.keys(value).forEach(name => {
     ...
     */
    getRawValue(): {[K in keyof TGroup]: TypeFromControl<TGroup[K]>} {
      return this._reduceChildren(
          {}, (acc: {[k: string]: AbstractControl}, control: AbstractControl, name: string) => {
            acc[name] =
                control instanceof FormControl ? control.value : (<any>control).getRawValue();
            return acc;
          });
    }
   ...
   * @publicApi
   */
  export class FormArray<TControl extends AbstractControl = AbstractControl> extends
      AbstractControl<Array<TypeFromControl<TControl>>> {
    /**
    ...
    */
    ...
    /**
     * Get the `TControl` at the given `index` in the array.
     *
     * @param index Index in the array to retrieve the control
     */
    at(index: number): TControl { return this.controls[index]; }

    /**
     * Insert a new `TControl` at the end of the array.
     *
    ...
    /**
     * Insert a new `TControl` at the given `index` in the array.
     *
     ...
     * @param control The `TControl` control to replace the existing control
     */
     ...
     */
    setValue(
        value: Array<TypeFromControl<TControl>>,
        options: {onlySelf?: boolean, emitEvent?: boolean} = {}): void {
      this._checkAllValuesPresent(value);
     ...
     * array.
     */
    getRawValue(): Array<TypeFromControl<TControl>> {
    ...
    clear(): void {
      ...
      this._forEachChild(
          (control: AbstractControl) => control._registerOnCollectionChange(() => {}));
    ...
    /** @internal */
    _forEachChild(cb: Function): void {
      this.controls.forEach((control: TControl, index: number) => { cb(control, index); });
    }
    ...
    /** @internal */
    _anyControls(condition: Function): boolean {
      return this.controls.some((control: TControl) => control.enabled && condition(control));
    }

    /** @internal */
    _setUpControls(): void {
      this._forEachChild((control: TControl) => this._registerControl(control));
    }

    /** @internal */
    _checkAllValuesPresent(value: any): void {
      this._forEachChild((control: TControl, i: number) => {
    ...
    private _registerControl(control: TControl) {
      control.setParent(this);
      control._registerOnCollectionChange(this._onCollectionChange);
    }
  }
