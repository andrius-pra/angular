 ...
 * @see `status`
 */
export const VALID = FormStatus.VALID;
 ...
 * @see `status`
 */
export const INVALID = FormStatus.INVALID;
 ...
 * @see `status`
 */
export const PENDING = FormStatus.PENDING;
 ...
 * @see `status`
 */
export const DISABLED = FormStatus.DISABLED;
...
export abstract class AbstractControl<T = any> {
   ...
   * false otherwise.
   */
  get valid(): boolean { return this.status === FormStatus.VALID; }
   ...
   */
  // TODO(issue/24571): remove '!'.
  public readonly statusChanges !: Observable<FormStatus>;
  ...
  markAsPending(opts: {onlySelf?: boolean, emitEvent?: boolean} = {}): void {
    (this as{status: string}).status = FormStatus.PENDING;
  ...
  disable(opts: {onlySelf?: boolean, emitEvent?: boolean} = {}): void {
    ...
    (this as{status: string}).status = FormStatus.DISABLED;
  ...
  enable(opts: {onlySelf?: boolean, emitEvent?: boolean} = {}): void {
    ...
    (this as{status: string}).status = FormStatus.VALID;
    this._forEachChild(
  ...
  updateValueAndValidity(opts: {onlySelf?: boolean, emitEvent?: boolean} = {}): void {
    ...
    if (this.enabled) {
      ...
      if (this.status === FormStatus.VALID || this.status === FormStatus.PENDING) {
  ...
  private _setInitialStatus() {
    (this as{status: string}).status = this._allControlsDisabled() ? FormStatus.DISABLED : FormStatus.VALID;
  }
  ...
  private _calculateStatus(): string {
    if (this._allControlsDisabled()) return FormStatus.DISABLED;
    if (this.errors) return FormStatus.INVALID;
    if (this._anyControlsHaveStatus(FormStatus.PENDING)) return FormStatus.PENDING;
    if (this._anyControlsHaveStatus(FormStatus.INVALID)) return FormStatus.INVALID;
...
export class FormGroup<T extends {[key:string]:any}= any> extends AbstractControl<T> {
  ...
  */
   ...
   */
  registerControl<U extends keyof T>(name: U, control: AbstractControl<T[U]>): AbstractControl {
   ...
   * @param control Provides the control for the given name
   */
  addControl<U extends keyof T>(name: U, control: AbstractControl<T[U]>): void {
   ...
   */
  removeControl<U extends keyof T>(name: U): void {
   ...
   * @param control Provides the control for the given name
   */
  setControl<U extends keyof T>(name: U, control: AbstractControl<T[U]>): void {
    if (this.controls[name]) this.controls[name]._registerOnCollectionChange(() => {});
    delete (this.controls[name]);
