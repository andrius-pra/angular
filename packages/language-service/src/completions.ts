...
function attributeCompletionsForElement(
  ...
  results.push(...angularAttributes(info, elementName, element));
...
function angularAttributes(
    info: AstResult, elementName: string, element?: Element): ts.CompletionEntry[] {
  element = element || new Element(elementName, [], [], null !, null, null);

  const {selectors, map: selectorMap} = getSelectors(info);
  const ngAttributes = new Map<string, AttrInfo>();
  for (const selector of selectors) {
    ...
    for (const attr of selector.attrs) {
      if (attr) {
        ngAttributes.set(attr, {
          name: attr,
          input: attr in summary.inputs,
          output: attr in summary.outputs,
          // Add template attribute if a directive contains a template reference
          template: hasTemplateReference(summary.type),
        });
      }
    }
  }
  ...
  matcher.match(createElementCssSelector(element), (selector: CssSelector) => {
    const summary = selectorMap.get(selector);
    if (summary) {
      for (const value of Object.values(summary.inputs)) {
        ngAttributes.set(value, {
          name: value,
          input: true,
        });
      }
      for (const value of Object.values(summary.outputs)) {
        ngAttributes.set(value, {
          name: value,
          output: true,
        });
  ...
  const results: ts.CompletionEntry[] = [];
  for (const value of ngAttributes.values()) {
    const {name, template, input, output} = value;
    results.push({
      name: template ? `*${name}` : input ? `[${name}]` : output ? `(${name})` : name,
      kind: CompletionKind.ATTRIBUTE as unknown as ts.ScriptElementKind,
      sortText: name,
    });
    if (ngAttributes.has(`${name}Change`)) {
