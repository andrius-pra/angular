...
class PlaceholderSubstituter {
  public static replacePlaceholders(
      typescript: typeof ts, settings: TemplateSettings,
      node: ts.TemplateExpression|ts.NoSubstitutionTemplateLiteral): string {
    ...
    return PlaceholderSubstituter.getSubstitutions(
        settings, literalContents, PlaceholderSubstituter.getPlaceholderSpans(node));
  }
  ...
  private static getSubstitutions(
      settings: TemplateSettings, contents: string,
      locations: ReadonlyArray<{start: number, end: number}>): string {
  ...
  private static getSubstitution(
      settings: TemplateSettings, templateString: string, start: number, end: number): string {
    return settings.getSubstitution ? settings.getSubstitution(templateString, start, end) :
                                      'x'.repeat(end - start);
  }
}
...
class StandardTemplateContext implements TemplateContext {
  constructor(
      public readonly typescript: typeof ts, public readonly fileName: string,
      public readonly node: ts.TemplateLiteral, private readonly helper: ScriptSourceHelper,
  ...
  public toOffset(position: ts.LineAndCharacter): number {
    const docOffset = this.helper.getOffset(
        this.fileName, position.line + this.stringBodyPosition.line, position.line === 0 ?
  ...
  public get text(): string {
    return PlaceholderSubstituter.replacePlaceholders(
        this.typescript, this.templateSettings, this.node);
  }
...
export default class StandardTemplateSourceHelper implements TemplateSourceHelper {
  constructor(
      ...
      private readonly helper: ScriptSourceHelper, _logger: Logger) {}

  public getTemplate(fileName: string, position: number): TemplateContext|undefined {
    const node = this.getValidTemplateNode(
        this.templateStringSettings, this.helper.getNode(fileName, position));
    if (!node) {
      return undefined;
    }
    ...
    return new StandardTemplateContext(
        this.typescript, fileName, node, this.helper, this.templateStringSettings);
  }

  public getAllTemplates(fileName: string): ReadonlyArray<TemplateContext> {
    const out: TemplateContext[] = [];
    for (const node of this.helper.getAllNodes(
             fileName,
             n => this.getValidTemplateNode(this.templateStringSettings, n) !== undefined)) {
      const validNode = this.getValidTemplateNode(this.templateStringSettings, node);
      if (validNode) {
        out.push(new StandardTemplateContext(
            this.typescript, fileName, validNode, this.helper, this.templateStringSettings));
      }
    }
    return out;
  }
  ...
  private getValidTemplateNode(templateStringSettings: TemplateSettings, node: ts.Node|undefined):
    ...
    switch (node.kind) {
      ...
      case this.typescript.SyntaxKind.NoSubstitutionTemplateLiteral:
        if (isTaggedLiteral(
                this.typescript, node as ts.NoSubstitutionTemplateLiteral,
                templateStringSettings.tags)) {
          return node as ts.NoSubstitutionTemplateLiteral;
        }
        return undefined;

      case this.typescript.SyntaxKind.TemplateHead:
        if (templateStringSettings.enableForStringWithSubstitutions && node.parent &&
            node.parent.parent) {
