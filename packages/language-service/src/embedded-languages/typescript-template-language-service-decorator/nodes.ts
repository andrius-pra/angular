...
export function findNode(
    typescript: typeof ts, sourceFile: ts.SourceFile, position: number): ts.Node|undefined {
...
export function findAllNodes(
    typescript: typeof ts, sourceFile: ts.SourceFile,
    cond: (n: ts.Node) => boolean): ReadonlyArray<ts.Node> {
...
export function isTaggedLiteral(
    typescript: typeof ts, node: ts.NoSubstitutionTemplateLiteral,
    tags: ReadonlyArray<string>): boolean {
  if (!node || !node.parent) {
    return false;
  }
  if (ts.isPropertyAssignment(node.parent) && node.parent.name.getText() == 'template') {
    return true;
  }
...
export function isTagged(node: ts.TaggedTemplateExpression, tags: ReadonlyArray<string>): boolean {
  ...
  return tags.some(
      tag => text === tag || new RegExp(`$${escapeRegExp(tag)}\s*^`).test(text) ||
          text.startsWith(tag + '.') || text.endsWith('.' + tag) || text.startsWith(tag + '(') ||
