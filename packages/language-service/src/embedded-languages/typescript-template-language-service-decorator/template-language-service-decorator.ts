...
export default class TemplateLanguageServiceProxy {
  private readonly _wrappers:
      Array<{name: keyof ts.LanguageService, wrapper: LanguageServiceMethodWrapper<any>}> = [];

  constructor(
      private readonly typescript: typeof ts, private readonly sourceHelper: TemplateSourceHelper,
      private readonly templateStringService: TemplateLanguageService, _logger: Logger) {
  ...
  private tryAdaptGetSyntaxDiagnostics() {
    ...
    const call =
        this.templateStringService.getSyntacticDiagnostics.bind(this.templateStringService);
  ...
  private tryAdaptGetQuickInfoAtPosition() {
    ...
    this.wrap(
        ...
        delegate => (fileName: string, position: number): ts.QuickInfo | undefined => {
          ...
          const quickInfo: ts.QuickInfo|undefined =
              this.templateStringService.getQuickInfoAtPosition !(
                  context, this.sourceHelper.getRelativePosition(context, position));
          if (quickInfo) {
  ...
  private tryAdaptGetCompletionsAtPosition() {
    ...
    this.wrap(
        ...
        delegate => (fileName: string, position: number, ...rest: any[]) => {
          const context = this.sourceHelper.getTemplate(fileName, position);
          if (!context) {
          ...
          return this.translateCompletionInfo(
              context, this.templateStringService.getCompletionsAtPosition !(
                           context, this.sourceHelper.getRelativePosition(context, position)));
        });
  }
  ...
  private tryAdaptGetCompletionEntryDetails() {
    ...
    this.wrap(
        ...
        delegate => (fileName: string, position: number, name: string, ...rest: any[]) => {
          const context = this.sourceHelper.getTemplate(fileName, position);
          if (!context) {
          ...
          return this.templateStringService.getCompletionEntryDetails !(
              context, this.sourceHelper.getRelativePosition(context, position), name);
        });
  }
  ...
  private tryAdaptGetFormattingEditsForRange() {
    ...
    this.wrap(
        ...
        delegate =>
            (fileName: string, start: number, end: number, options: ts.FormatCodeSettings) => {
              ...
              for (const template of this.sourceHelper.getAllTemplates(fileName)) {
                ...
                for (const change of this.templateStringService.getFormattingEditsForRange !(
                         template, templateStart, templateEnd, options)) {
  ...
  private tryAdaptGetCodeFixesAtPosition() {
    ...
    this.wrap(
        ...
        delegate =>
            (fileName: string, start: number, end: number, errorCodes: ReadonlyArray<number>,
             options: ts.FormatCodeSettings, preferences: ts.UserPreferences) => {
              ...
              for (const template of this.sourceHelper.getAllTemplates(fileName)) {
                ...
                const templateStart = Math.max(0, start - nodeStart);
                const templateEnd = Math.min(nodeEnd - nodeStart, end - nodeStart);
                for (const codeAction of this.templateStringService.getCodeFixesAtPosition !(
                         template, templateStart, templateEnd, errorCodes, options)) {
    ...
    this.wrap(
        ...
        delegate => (fileName: string, position: number, ...rest: any[]) => {
          ...
          if (context) {
            ...
            return definition ? definition.map(def => this.translateDefinitionInfo(context, def)) :
                                undefined;
          }
    ...
    this.wrap(
        ...
        delegate => (fileName: string, position: number, ...rest: any[]) => {
          ...
          const signatureHelp = this.templateStringService.getSignatureHelpItemsAtPosition !(
              context, this.sourceHelper.getRelativePosition(context, position));
          return signatureHelp ? this.translateSignatureHelpItems(context, signatureHelp) :
                                 undefined;
        });
  }
  ...
  private tryAdaptGetReferencesAtPosition() {
    ...
    this.wrap(
        'findReferences',
        delegate => (fileName: string, position: number, ...rest: any[]): ts.ReferencedSymbol[] |
            undefined => {
          const context = this.sourceHelper.getTemplate(fileName, position);
          if (context) {
  ...
  private tryAdaptGetJsxClosingTagAtPosition() {
    ...
    this.wrap(
        ...
        delegate => (fileName: string, position: number, ...rest: any[]): ts.JsxClosingTagInfo |
            undefined => {
          const context = this.sourceHelper.getTemplate(fileName, position);
          if (context) {
  ...
  private wrap<K extends keyof ts.LanguageService>(
      name: K, wrapper: LanguageServiceMethodWrapper<K>) {
  ...
  private adaptDiagnosticsCall(
      ...
      implementation: (context: TemplateContext) => ts.Diagnostic[], fileName: string) {
  ...
  private translateCompletionInfo(context: TemplateContext, info: ts.CompletionInfo):
      ts.CompletionInfo {
    return {
      ...info,
  ...
  private translateCompletionEntry(context: TemplateContext, entry: ts.CompletionEntry):
      ts.CompletionEntry {
    return {
      ...entry,
      replacementSpan: entry.replacementSpan ?
          this.translateTextSpan(context, entry.replacementSpan) :
          undefined,
    };
  }

  private translateTextChange(context: TemplateContext, textChange: ts.TextChange): ts.TextChange {
    return {
      ...textChange,
  ...
  private translateFileTextChange(context: TemplateContext, changes: ts.FileTextChanges):
      ts.FileTextChanges {
    return {
      fileName: changes.fileName,
      textChanges:
          changes.textChanges.map(textChange => this.translateTextChange(context, textChange)),
    };
  }

  private translateCodeAction(context: TemplateContext, action: ts.CodeFixAction|ts.CodeAction):
      ts.CodeFixAction {
    return {
      ...action,
  ...
  private translateSignatureHelpItems(
      context: TemplateContext, signatureHelp: ts.SignatureHelpItems): ts.SignatureHelpItems {
    return {
      ...signatureHelp,
  ...
  private translateOutliningSpan(context: TemplateContext, span: ts.OutliningSpan):
      ts.OutliningSpan {
    return {
      ...span,
  ...
  private translateTextSpan(context: TemplateContext, span: ts.TextSpan): ts.TextSpan {
    return {
  ...
  private translateDefinitionInfo(context: TemplateContext, definition: ts.DefinitionInfo):
      ts.DefinitionInfo {
    return {
      ...definition,
  ...
  private translateReferenceEntry(context: TemplateContext, entry: ts.ReferenceEntry):
      ts.ReferenceEntry {
    return {
      ...entry,
      fileName: context.fileName,
