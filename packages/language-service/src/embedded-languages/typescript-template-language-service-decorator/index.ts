...
export function decorateWithTemplateLanguageService(
    typescript: typeof ts, languageService: ts.LanguageService, project: ts.server.Project,
    templateService: TemplateLanguageService, templateSettings: TemplateSettings,
    additionalConfig?: AdditionalConfiguration): ts.LanguageService {
  const logger = (additionalConfig && additionalConfig.logger) || nullLogger;
  return new TemplateLanguageServiceDecorator(
             typescript, new StandardTemplateSourceHelper(
                             typescript, templateSettings,
                             new StandardScriptSourceHelper(typescript, project), logger),
